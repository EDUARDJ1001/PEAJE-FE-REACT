...
const TicketsEmpleadosV3: React.FC = () => {
    ...
    const obtenerUltimoId = async () => {
        try {
            const response = await fetch(`${apiHost}/api/contador-v3/ultimo-id`);
            if (!response.ok) {
    ...
    const actualizarConteoBoletos = async (boleto: Boleto) => {
        try {
            const insertarResponse = await fetch(`${apiHost}/api/contador-v3/insert`, {
                method: "POST",
                headers: {
...
export default TicketsEmpleadosV3;
